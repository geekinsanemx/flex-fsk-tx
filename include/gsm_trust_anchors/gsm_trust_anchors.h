#ifndef GSM_TRUST_ANCHORS_H
#define GSM_TRUST_ANCHORS_H

#ifdef __cplusplus
extern "C" {
#endif

/* Amazon Root CA 1 trust anchor generated via SSLClient pycert_bearssl tool */

#define GSM_TAS_NUM 1

static const unsigned char GSM_TA_DN0[] = {
    0x30, 0x39, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13,
    0x02, 0x55, 0x53, 0x31, 0x0f, 0x30, 0x0d, 0x06, 0x03, 0x55, 0x04, 0x0a,
    0x13, 0x06, 0x41, 0x6d, 0x61, 0x7a, 0x6f, 0x6e, 0x31, 0x19, 0x30, 0x17,
    0x06, 0x03, 0x55, 0x04, 0x03, 0x13, 0x10, 0x41, 0x6d, 0x61, 0x7a, 0x6f,
    0x6e, 0x20, 0x52, 0x6f, 0x6f, 0x74, 0x20, 0x43, 0x41, 0x20, 0x31,
};

static const unsigned char GSM_TA_RSA_N0[] = {
    ,
};

static const unsigned char GSM_TA_RSA_E0[] = {
    0x01, 0x00, 0x01,
};

static const br_x509_trust_anchor GSM_TAs[] = {
    {
        { (unsigned char *)GSM_TA_DN0, sizeof GSM_TA_DN0 },
        BR_X509_TA_CA,
        {
            BR_KEYTYPE_RSA,
            { .rsa = {
                (unsigned char *)GSM_TA_RSA_N0, sizeof GSM_TA_RSA_N0,
                (unsigned char *)GSM_TA_RSA_E0, sizeof GSM_TA_RSA_E0,
            } }
        }
    },
};

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif /* GSM_TRUST_ANCHORS_H */
